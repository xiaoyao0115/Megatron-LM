...
def _should_eager_initialize_dynamic_cp_communicators(args):
    """Keep logical CP communicators eager unless captured attention prepares their path."""
    graph_modules = getattr(args, 'cuda_graph_modules', ())
    captures_attention = not graph_modules or CudaGraphModule.attn in graph_modules
    return not (
        args.dynamic_context_parallel
        and args.cuda_graph_impl == 'transformer_engine'
