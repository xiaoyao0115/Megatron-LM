...
class GatedDeltaNet(MegatronModule):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Resolve runtime CP group and size (dynamic CP uses packed_seq_params.cp_group)
        ...
        # Validate that cp_size evenly divides head counts for head-parallel CP
        num_key_heads_per_tp = self.num_key_heads // self.tp_size
        num_value_heads_per_tp = self.num_value_heads // self.tp_size
        assert num_key_heads_per_tp % cp_size == 0, (
            f"GDN head-parallel CP requires cp_size ({cp_size}) to evenly divide "
            f"num_key_heads per TP rank ({num_key_heads_per_tp}). "
            f"With dynamic CP, restrict max_cp_size to {num_key_heads_per_tp}."
        )
        assert num_value_heads_per_tp % cp_size == 0, (
            f"GDN head-parallel CP requires cp_size ({cp_size}) to evenly divide "
            f"num_value_heads per TP rank ({num_value_heads_per_tp}). "
            f"With dynamic CP, restrict max_cp_size to {num_value_heads_per_tp}."
        )
        ...
        if self.config.deterministic_mode:
            ...
            # Pad sequence to reduce Triton autotune key variability.
            # NB = cdiv(total_tokens, 1024) is part of the autotune key;
            # aligning total_tokens to 4096 collapses NB to fewer buckets.
            _conv_pad_align = 4096
            _orig_seq = qkv.shape[1]
            _pad_n = (_conv_pad_align - _orig_seq % _conv_pad_align) % _conv_pad_align
            _conv_input = qkv
