...
class TECudaGraphHelper:
    ...
    def __init__(
        ...
        # Initialize every parent communicator before graph capture. Captured attention and
        # router ops also route eager warmup iterations through that parent, otherwise the
        ...
        if self._reuse_parent_cp_transport or warmup_parent_router:
            try:
                self._set_dynamic_cp_parent_router_reduction(warmup_parent_router)
                ...
                self._release_dynamic_cp_graph_communication()
    ...
    def _should_reuse_dynamic_cp_p2p_transport(self) -> bool:
        ...
        return (
            self._should_share_dynamic_cp_pool()
            and captures_attention
    ...
    def _release_dynamic_cp_p2p_transport(self):
        """Release Python transport overrides after setup or capture."""
        if not self._dynamic_cp_transport_contexts:
            return
        self._set_dynamic_cp_p2p_transport(self._dynamic_cp_transport_contexts, None)
        self._dynamic_cp_transport_contexts = ()

    def _release_dynamic_cp_graph_communication(self):
        """Release capture-only attention and router communication state."""
        try:
            self._release_dynamic_cp_p2p_transport()
        finally:
            self._set_dynamic_cp_parent_router_reduction(False)
    ...
    def _set_dynamic_cp_parent_router_reduction(self, enabled):
        """Scope parent router reductions to graph warmup and capture."""
        self._set_cuda_graph_config_attr(
            '_cuda_graph_parent_router_reduction', True if enabled else None
        )
    ...
    def create_cudagraphs(self):
        ...
        try:
            ...
            use_parent_router = self._should_use_dynamic_cp_parent_router_reduction()
            self._set_dynamic_cp_parent_router_reduction(use_parent_router)
            if use_parent_router:
                self._warmup_dynamic_cp_parent_collective(self.dp_cp_group)
            self._release_dynamic_cp_p2p_transport()
            if self._should_reuse_dynamic_cp_p2p_transport():
                if reuse_parent_cp_transport:
                    self._dynamic_cp_transport_contexts = tuple(capture_contexts)
                    self._set_dynamic_cp_p2p_transport(
                        self._dynamic_cp_transport_contexts, self.dp_cp_group
                    )
                self._warmup_dynamic_cp_communicators(
            ...
            # NCCL communicator handles are embedded in the captured graph nodes. The Python
            # override is only needed while warming and capturing those nodes; keeping it active
            # would also reroute unrelated eager attention calls for the rest of training.
            self._release_dynamic_cp_graph_communication()
