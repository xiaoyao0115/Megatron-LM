...
class TestDynamicMicrobatchSlots:
    ...
    @pytest.mark.internal
    @pytest.mark.parametrize(
        ("dynamic_cp", "graph_impl", "graph_modules", "cp_comm_type", "expected"),
        (
            (True, "transformer_engine", [], ["p2p"], False),
            (True, "transformer_engine", ["attn"], ["p2p"], False),
            (True, "transformer_engine", ["attn"], ["a2a"], True),
            (True, "transformer_engine", ["attn"], ["all_gather"], True),
            (True, "transformer_engine", ["moe_router"], ["p2p"], True),
            (True, "none", [], ["p2p"], True),
            (False, "transformer_engine", [], ["p2p"], True),
        ),
    )
    def test_logical_cp_communicator_init_follows_graph_attention_scope(
        self, dynamic_cp, graph_impl, graph_modules, cp_comm_type, expected
    ):
        from megatron.core.transformer.enums import CudaGraphModule
        from megatron.training.initialize import (
            _should_eager_initialize_dynamic_cp_communicators,
        )

        args = SimpleNamespace(
            dynamic_context_parallel=dynamic_cp,
            cuda_graph_impl=graph_impl,
            cuda_graph_modules=[CudaGraphModule[name] for name in graph_modules],
    ...
    def test_router_only_graph_warms_parent_collective_without_p2p(self, monkeypatch):
        ...
        assert config._cuda_graph_parent_router_reduction is True
    ...
    @pytest.mark.internal
    def test_successful_capture_releases_python_p2p_transport_override(self):
        from megatron.core.transformer.cuda_graphs import TECudaGraphHelper
        ...
        helper._dynamic_cp_transport_contexts = tuple(capture_contexts)
        ...
        helper._should_use_dynamic_cp_parent_router_reduction = lambda: False
        helper._should_share_dynamic_cp_pool = lambda: False
        helper._warmup_dynamic_cp_communicators = lambda contexts, group: None
        helper._warmup_dynamic_cp_parent_collective = lambda group: None
        ...
        helper._abort_capturing = lambda captured_graphs: None
        helper.flattened_callables = []
        helper.dp_cp_group = object()

        transport_calls = []
        helper._set_dynamic_cp_p2p_transport = lambda contexts, group: transport_calls.append(
            (tuple(contexts), group)
        )
        ...
        assert transport_calls == [
            (tuple(capture_contexts), None),
            (tuple(capture_contexts), helper.dp_cp_group),
            (tuple(capture_contexts), None),
        ]
        assert helper._dynamic_cp_transport_contexts == ()
    ...
    def test_failed_capture_retry_restores_capture_only_state(self):
        ...
        helper._capture_finished = False
        helper._reuse_parent_cp_transport = True
        helper._dynamic_cp_transport_contexts = ()
        ...
        helper.flattened_callables = []
        helper.dp_cp_group = object()
        helper.config = SimpleNamespace(_cuda_graph_parent_router_reduction=True)
        helper.chunks_with_decoder = []
        ...
        def start_capturing():
            assert helper.config._cuda_graph_parent_router_reduction is True
        ...
        assert not hasattr(helper.config, '_cuda_graph_parent_router_reduction')
