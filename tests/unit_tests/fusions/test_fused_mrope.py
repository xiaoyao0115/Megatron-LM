...
def test_materialized_mrope_falls_back_without_te_fused_rope(monkeypatch, layout):
    ...
    max_seqlen = None if cu_seqlens is None else _max_seqlen(cu_seqlens)
    emb = mrope_freqs_to_rotary_emb(freqs, mrope_section, rotary_interleaved=False)

    with pytest.warns(UserWarning, match="Transformer Engine fused RoPE.*unavailable"):
        out = apply_rotary_pos_emb(
            t, emb, config, cu_seqlens, cp_group=FakeCPGroup(), max_seqlen=max_seqlen
        )
    ...
    if layout == "bshd":
        ...
        ref = _apply_rotary_pos_emb_thd(
            t, cu_seqlens, emb, cp_group=FakeCPGroup(), max_seqlen=max_seqlen
        )
    torch.testing.assert_close(ref.float(), out.float(), **_dtype_tols(t.dtype))
...
def test_materialized_thd_short_freqs_fall_back_without_calling_te(
    monkeypatch, cp_size, cp_rank
):
    ...
    freqs = torch.randn(
        4, 1, 1, 16, dtype=torch.float32, device="cuda", generator=generator
    )
    ...
    with warnings.catch_warnings(record=True) as recorded_warnings:
        ...
        out = apply_rotary_pos_emb(
            ...
            cu_seqlens,
            cp_group=cp_group,
            max_seqlen=global_token_capacity,
        )

    fallback_warnings = _fallback_warnings(recorded_warnings)
    assert len(fallback_warnings) == 1
    assert "frequency table length (4) to cover max_seqlen (8)" in str(
...
def test_materialized_thd_short_freqs_fallback_captures_and_replays(monkeypatch):
    ...
    with torch.cuda.stream(warmup_stream), pytest.warns(
        UserWarning, match="frequency table length"
    ):
...
def test_fused_mrope_thd_matches_unfused_forward_backward(
    ...
    ref = _apply_rotary_pos_emb_thd(
        t_ref, cu_seqlens, emb, cp_group=cp_group, max_seqlen=_max_seqlen(cu_seqlens)
    )
...
def test_apply_rotary_pos_emb_thd_eval_uses_triton_without_te(interleaved_mrope, monkeypatch):
    ...
    ref = _apply_rotary_pos_emb_thd(
        t, cu_seqlens, emb, cp_group=FakeCPGroup(), max_seqlen=_max_seqlen(cu_seqlens)
    )
    ...
    with torch.no_grad(), warnings.catch_warnings(record=True) as recorded_warnings:
        warnings.simplefilter("always")
        out = apply_rotary_pos_emb(
            t, freqs, config, cu_seqlens, cp_group=FakeCPGroup(), max_seqlen=_max_seqlen(cu_seqlens)
        )
...
def test_fused_mrope_thd_fp32_compute_matches_explicit_cast_forward_backward():
    ...
    ref = _apply_rotary_pos_emb_thd(
        t_ref.float(), cu_seqlens, emb, cp_group=FakeCPGroup(), max_seqlen=_max_seqlen(cu_seqlens)
    ).to(t_ref.dtype)
